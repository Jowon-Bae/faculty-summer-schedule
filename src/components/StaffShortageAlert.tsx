...
export function StaffShortageAlert() {
  const { minStaffCount, shortageDaysStr } = useMemo(() => {
    ...
    const formattedDays = minDays.map(d => format(d, 'M/d(E)', { locale: ko })).join(', ');
  ...
  return (
    <div className="fade-in" style={{
      ...
      <div style={{ fontSize: '0.875rem', lineHeight: '1.4' }}>
        <strong>⚠️ 교직원 공백 알림</strong><br/>
        여름 일정 중 가장 인원이 적은 날은 <strong>{shortageDaysStr}</strong> (잔여 {minStaffCount}명) 입니다.
      </div>
    </div>
  );
}
